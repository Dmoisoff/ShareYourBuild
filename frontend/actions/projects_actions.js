...
export const fetchProject = id => {
  return dispatch => {
    return Projects_Util.fetchProject(id).then(
      payload => {
        dispatch({
          type: FETCH_PROJECT,
...
export const createProject = project => {
  return dispatch => {
    return Projects_Util.createProject(project).then(
      payload => {
        dispatch({
...
export const updateProject = (project, id) => {
  return dispatch => {
    return Projects_Util.updateProject(project, id).then(
      payload => {
        dispatch({
          type: FETCH_PROJECT,
          project: payload.project
...
export const deleteProject = id => dispatch => {
  return Projects_Util.deleteProject(id).then(deletedId =>
    dispatch({ type: REMOVE_PROJECT, projectId: deletedId })
  );
};
